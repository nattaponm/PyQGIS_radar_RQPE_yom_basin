'''
20200612
ฝนสะสมราย6ชั่วโมงของสถานีที่มีข้อมูลฝนสมบูรณ์ ปรับปรุงจาก "108"
...
outpRain6h='D:/tmp/PyQGIS/Plugin_practice/z_temp/1outp_gauge_temp/1gauge6h/'#เป็นโฟลเดอร์เก็บข้อมูลผลลัพธ์ฝนราย6ชมของสถานีที่สมบูรณ์ที่ได้จากไฟล์"105"
...
print('เริ่มการสะสมฝน6ชั่วโมงจากการดึงข้อมูลฝนราย15นาทีของแต่ละสถานีฝน..............')
#วันเวลาที่ต้องการรวมไฟล์ฝนราย6ชั่วโมง
...
for dd in time: #วัน
    ...
    for n in range(4): #4 คือ 24/6 สะสมฝนทุกๆ6ชม.
        print('ทำการสะสมฝนราย6ชั่วโมง...'+dd+"{:02d}".format(n*6))
        start=n*6
        stop=start+6
        
        gg=[] #ลิสเก็บชื่อไฟล์ที่หาได้ในแต่ละชั่วโมงที่ต้องการ ในที่นี้ทีละ6ชั่วโมง
        ...
        for s in st:#สถานีวัดฝนที่สมบูรณ์จากไฟล์"105"
            ...
            if os.path.isfile(gf): #ตรวจว่ามีข้อมูลไฟล์สถานีไหม
                ...
                sum_rr=0.0
                i=1
                for hh in range(start,stop): #ลูป6ชั่วโมงที่ต้องการ           
                        
                    pat=dd[6:]+'/'+dd[4:6]+'/'+dd[0:4]+' '+"{:02d}".format(hh) #ใช้คอลัมน์1 ของ g
                    if i==1:hh_start=dd+"{:02d}".format(start) #ชื่อไฟล์ชั่วโมงแรกของ6ชั่วโมงที่กำลังสะสม
...
#                    print('.....',len(data),rr,sum_rr)                    
                ...
                #เก็บข้อมูลฝนสะสมราย6 ชั่วโมงที่ดึงมาจากฝน15นาทีมาเก็บในลิสจนครบทุกสถานี
                gg.append((data))
                    
        #เซฟ gg เป็น csv ตามชื่อไฟล์
        #เซฟเป็นไฟล์ csv ของเวลาราย15นาทีนั้นๆ tt+'.csv'
        gf = open(outpRain6h+hh_start+'.csv', 'w',newline='') #newline='' เพื่อป้องกันการเพิ่มบรรทัดตอนเซฟ csv
        with gf:
            w = csv.writer(gf)
            w.writerows(gg)         
#        break
#    break
print('เสร็จสิ้นการสะสมฝน6ชั่วโมงจากการดึงข้อมูลฝนราย15นาทีของแต่ละสถานีฝน..............')
