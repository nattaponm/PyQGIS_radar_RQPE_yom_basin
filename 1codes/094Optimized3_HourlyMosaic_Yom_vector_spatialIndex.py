'''
20200609 โมเสคฝนสะสมรายชั่วโมง ใช้โค้ดนี้ได้ ยืนยัน คำนวนฝนสะสมบนหลักของเวกเตอร์ ซึ่งช้า จะเปลี่ยนเป็นราสเตอร์
ดัดแปลงโค้ดจาก "093" ดูฟังค์ชั่น accumulateRain2Hr(pLayer,rainRad)ใช้ spatial index
อ่าน https://gis.stackexchange.com/questions/130439/how-to-efficiently-access-the-features-returned-by-qgsspatialindex/130622
โค้ด"092" นั้นอย่าใช้ เพราะการรวมฝนสะสมผิด ให้ใช้ไฟล์นี้แทน
ไฟล์นี้จะใช้เป็นโค้ดหลักในการทำฝนสะุสมรายชั่วโมง เพื่อนำไปหาฝนสะสมราย 3 ชั่วโมง,6,12,24 ต่อไป โดยจะทำในระบบราสเตอร์
ใช้ processing toolbox ในการรวมฝน

#ต่อไปคือแปลงชื่อฝนแบบ UTC เป็น local 
...
def Mosaic_CRI_PHS(Flare_BB_CRI,Flare_BB_PHS,path_to_bound_layer,ds_s,epsg,rad_name1,rad_name2,file_time):
    ...
    pr_c.addAttributes([QgsField("Area", QVariant.Double,'double', 10, 3),
                      ...
                      QgsField("Rain00", QVariant.Double,'double', 10, 3), #ตรงนี้จะใช้เป็นคอลัมน์ที่get ค่าไปใส่ในคอลัมน์ถัดไป
                      QgsField("Rain15", QVariant.Double,'double', 10, 3),
                      QgsField("Rain30", QVariant.Double,'double', 10, 3),
                      QgsField("Rain45", QVariant.Double,'double', 10, 3),
                      QgsField("Rain1h", QVariant.Double,'double', 10, 3)])
...
def reprojectRad(path_rad,rad_name):
    ...
    return path_rad
#-------------------------------------------------------------------------------
def convertdbZ2RainRadar(a,b,pLayer,num_files):
...
#    for ft in pLayer.selectedFeatures():
    for ft in pLayer.getFeatures():
        if ft['Val']==-999:
            ...
            # 5="Rain00" แต่เดิมเป็น "RainRad"
            pLayer.changeAttributeValue(ft.id(),5,r/num_files) # r/num_files มีหน่วย mm/hr เพราะ 1ชั่วโมงมีหลายไฟล์
...
#-------------------------------------------------------------------------------
def accumulateRain2Hr(pLayer,rainRad,k,num_files):
    col=k+5 #เอาไว้เช็ค
    #สร้างฝนสะสม ตรงส่วนนี้ใช้ spatial index ต่างกับ "092" ทำให้เร็วขึ้นชัดเจนมากๆ
    print('>>>สร้างฝนสะสม........')    
    #ใช้spatial index เพื่อเก็บค่าของ rainRad
    all_features = {}
    index = QgsSpatialIndex() # Spatial index
    for ft in rainRad.getFeatures(): #เก็บค่าฟีเจอร์ของ rainRad แบบ spatial index
        index.insertFeature(ft)
        all_features[ft.id()] = ft    

    #ลูปเพื่อหาว่ามี geometry ที่เท่ากัน เพราะกริดต้องเท่ากัน 
    #อ่าน https://gis.stackexchange.com/questions/130439/how-to-efficiently-access-the-features-returned-by-qgsspatialindex/130622
    pLayer.startEditing() #ตั้งค่าให้เริ่มการแก้ไขชั้นขัอมูล
    i=0
    for feat in pLayer.getFeatures():
        ids = index.intersects(feat.geometry().boundingBox())#หา intersect ก่อนจะได้ลิสหาจุด
        for id in ids:
            ft = all_features[id]
            if ft.geometry().equals(feat.geometry()): #การใช้ euals จะกรองเอาที่มี geometry ที่เท่ากันจริงๆ
                if not ft['Rain00']:
                    rr=0
                else:                    
                    rr=ft['Rain00']

                pLayer.changeAttributeValue(feat.id(),col,rr) #  col คือคอลัมน์ ราย15นาที ต้องการนำมาตรวจสอบการสะสม    
                
                #ถ้าเป็นไฟล์สุดท้าย45 ให้รวมฝนแล้วอัพเดทในคอลัมน์สุดท้ายของไฟล์ผลลัพธ์ลัพธ์
                if k==(num_files-1):                    
                    if not feat['Rain00']:                        
                        r00=0
                    else:
                    ...
                    if not ft['Rain00']:  #สังเกตใช้ ft ไม่ใช่ feat                      
                        r45=0
                    else:
                        r45=ft['Rain00']
                
                    r_acc=r00+r15+r30+r45                    
                    pLayer.changeAttributeValue(feat.id(),col+1,r_acc)
                break    
    ...
    for feat in pLayer.getFeatures():
        ...
        for id in idsList:          
            if feat['Val']==-999 and all_features[id]['heightASL']<5000 and all_features[id]['Flare1']>0:
                pLayer.changeAttributeValue(feat.id(),9,-999) #flare สังเกตเปลี่ยนจาก5เป็น9
                break
...
#โค้ดหลัก
print('#'*50)
print('เริ่มการประมวลผลทำฝนสะสมรายชั่วโมงจากการโมเสคราย 15 นาที...................')

#เตรียมชื่อไฟล์เรดารเชียงราย
rad_cri = 'D:/tmp/PyQGIS/Plugin_practice/1data/1rad_sontihn/shp15Test/CRI/'
fn1 = []
...
rad_phs = 'D:/tmp/PyQGIS/Plugin_practice/1data/1rad_sontihn/shp15Test/PHS/'
fn2 = []
fn2 += [f for f in os.listdir(rad_phs) if f.endswith('.shp')] #ลิสชื่อไฟล์เต็ม
ft2=[]
ft2+=[f[3:15]for f in fn2 ] #ลิสเวลาจากชื่อไฟล์์ราย15นาที่
################################################################################
#เตรียมชื่อไฟล์รายชั่วโมงเรดารเชียงราย
hr1=[]
hr1+=[f[0:10]for f in ft1 ] #ลิสเวลารายชั่วโมง
fh1=list(set(hr1)) #ลิสของฝนรายชั่วโมง
fh1.sort()

#เตรียมชื่อไฟล์รายชั่วโมงเรดาร์พิษณุโลก
hr2=[]
hr2+=[f[0:10]for f in ft2 ] #ลิสเวลารายชั่วโมง
fh2=list(set(hr2)) #ลิสของฝนรายชั่วโมง
fh2.sort()

################################################################################
#ลูปลิสฝนรายชั่วโมง เพื่อสร้างฝนสะสม
for fh in fh1:
    pat = '*'+fh+'*.shp' #
    file1 = fnmatch.filter(fn1, pat) #CRI
    file2 = fnmatch.filter(fn2, pat) #PHS
    
    if len(file1)==len(file2): #ถ้าจำนวนไฟล์ในชั่วโมงนั้นๆเท่ากัน
        num_files=len(file1)
        print('ดำเนินการโมเสคไฟล์ในชั่วโมง...'+fh)
        i=0
        for n in range(num_files): #ลูปไฟล์ในชั่วโมงนั้นๆ
            print('>>> ',fh)
            path_rad = "D:/tmp/PyQGIS/Plugin_practice/1data/1rad_sontihn/shp15Test/CRI/"+file1[n]
            path_beamblock = "D:/tmp/PyQGIS/Plugin_practice/1data/BeamBlockChiangraiRaster.tif"
            ...
            path_rad = "D:/tmp/PyQGIS/Plugin_practice/1data/1rad_sontihn/shp15Test/PHS/"+file2[n]
            path_beamblock = "D:/tmp/PyQGIS/Plugin_practice/1data/BeamBlockPhitsanulokRadarRaster.tif"
            rad_name='PHS'
            epsg = 32647 #กำหนดค่าพิกัดชั้นข้อมูลที่จะใช้WGS84
            binSize=1000.0
            ...
            ################################################################################
            #2.โมเสคเรดาร์
            path_to_bound_layer = "D:/tmp/PyQGIS/Plugin_practice/1data/Yom_Basin_utm.shp"
            ...
            stt=Timing_start()
            mosaic_cri_phs=Mosaic_CRI_PHS(Flare_BB_CRI,Flare_BB_PHS,path_to_bound_layer,ds_s,epsg,rad_name1,rad_name2,fh)
            print('Mosaic_CRI_PHS(Flare_BB_CRI,Flare_BB_PHS,path_to_bound_layer,ds_s,epsg,rad_name1,rad_name2,fh).......... ')
            Timing_used(stt)
            #-------------------------------------------------------------------------------
            ...
            stt=Timing_start()
            rainRad=convertdbZ2RainRadar(a,b,mosaic_cri_phs,num_files) #num_files เพื่อเอาไปหารฝนให้กลายเป็นฝนรายชั่วโมง
            print('convertdbZ2RainRadar(a,b,mosaic_cri_phs)..........')
            Timing_used(stt)
            
            #-------------------------------------------------------------------------------
            #4.สร้างฝนสะสมรายชั่วโมง
            stt=Timing_start()
            if i==0:#ไฟล์แรกจะถูกใช้เป็นไฟล์ผลลัพธ์
                acc_rad=rainRad
                i+=1
                continue
            else:
                #ฟังค์ชั่นฝนสะสมรายชั่วโมง
                acc_rad=accumulateRain2Hr(acc_rad,rainRad,i,num_files) #i จะนำไปใช้ในการอัพเดทคอลัมน์ของฝน15,30,45 เพื่อตรวจสอบผลการสะสม
                i+=1
            print('accumulateRain2Hr(acc_rad,rainRad)'+'...'+fh)
            Timing_used(stt)
        #-------------------------------------------------------------------------------
        ...
        #pLayer=rainRad
        stt=Timing_start()
        acc_rad=updateColumnFlare2Rainrad(acc_rad,Flare_BB_CRI)
        print('updateColumnFlare2Rainrad(acc_rad,Flare_BB_CRI)..........CRI')
        Timing_used(stt)

        stt=Timing_start()
        final_accRad=updateColumnFlare2Rainrad(acc_rad,Flare_BB_PHS)
        print('updateColumnFlare2Rainrad(acc_rad,Flare_BB_PHS)..........PHS')
        Timing_used(stt)
        #-------------------------------------------------------------------------------
        #5.ส่งออกผลลัพธ์ฝนประมาณค่าเป็น geotif เพื่อนำไป validate กับฝนสถานี
        outp_file=fh+".tif"
        outp='D:/tmp/PyQGIS/Plugin_practice/z_temp/1outp_mosaic_temp/'+outp_file
        stt=Timing_start()
        exportRainEstimates2Tif(outp,final_accRad)
        print('exportRainEstimates2Tif(outp)..........')
        Timing_used(stt)
        #-------------------------------------------------------------------------------
        ################################################################################
        #6.แสดงผลในแผนที่
        QgsProject.instance().addMapLayers([final_accRad])
        #-------------------------------------------------------------------------------
        print(' จบการประมวลผลไฟล์'+fh+'.....')
        print('#-'*25)
        
print('สิ้นสุดการประมวลผลฝนสะสมรายชั่วโมง...................')
print('#'*50)   
